...
\part{Protokół o organizacji bitowej SDLC (Synchronous Data Link Control)}
	...
	\section{Zapobieganie kolizji}
		...
		\subsection{Algorytmy z przekazywaniem uprawnień (Token)}
			...
			\subsection{Sieć pierścieniowa (Token Ring)}
				...
				\end{itemize}
				\subsubsection{Sieć pierścieniowa dla CSMA / CD}
					\begin{itemize}
						\item Liczba bitów w pierścieniu $ N_{ring}=\frac{L}{\frac{2}{3}C}\times \frac{1}{V}$, gdzie: \emph{L} to długość kabla łącza; \emph{V} to szybkość nadawania; \emph{C} to prędkość światła
						\item Liczba bitów pamiętanych na kablach $ n=\frac{T_p}{T_b} $
						\item Dla Cambridge Ring: $ N_{ramek}=N_{ring}+N_{stacji}=\sfrac{4N}{40bit} $
					\end{itemize}
				\subsubsection{Sieć dwupierścieniowa (Token Ring)}
...
\subsubsection{Parametry czasowe}
	\begin{itemize}
		\item refresh - czas odświeżania, co jaki czas secondary pyta primary
		\item retry - czas po którym następuje kolejna próba połączenia, powtórzenie refreshu, aż się pliki pobierze, albo aż minie expire.
		\item expire - czas po którym połączenie się poddaje
		\item TTL - czas życia w pamięci notatnikowej, jak DNS go pobierze do na tyle czasu trwa w DNS
	\end{itemize}
\subsubsection{Name Serwery}
	\begin{itemize}
		\item Primary
		\item Secondary
		\item Resolver - klient DNS
		\item Couching - primary lub secondary. Serwer notatnikowy.
	\end{itemize}

\section{Poczta elektorniczna}
	\subsection{Historia}
		Rok 1982: rdc 822 - definiuje metody komunikacji między klientami, tzw. systemy agentowe.\\
		Powstał jako wersja przejściowa wielkich standardów ISO.
		\begin{itemize}
			\item user-agent
			\item mail-transfer-agent
		\end{itemize}
		Bardzo trwała prowizorka.
	\subsection{STMP}
		\emph{Simple Transter Mail Protocol}. Przypisany do portu 25.
		\subsubsection{Adres użytkownika}
			\begin{itemize}
				\item Zbiór par: atrybut = wartość
			\end{itemize}
		\subsubsection{przykład}
			...
			\begin{itemize}
				\item SL - slot
				\item L - locality
				\item PA - present address
			\end{itemize}
			\includegraphics[width=9cm]{./images/drawing10.pdf}
			\subsubsection{Nagłówki}
				Nagłówek: [do 1024 znaków][znak ENTER]
				\begin{itemize}
					\item To: 
					\item Cc: (\emph{carbon copy}) - wysyła kopie listu
					\item Bcc: (\emph{black carbon copy}) - to co wyżej, ale odbiorcy są niewidoczni (??)
					\item From:
					\item Sender: - autor listu np. sekretarz, nie prezes
					\item Reply-to:
					\item Received:
					\item Return-path: - informacja pozwalająca odesłać przesyłkę
				\end{itemize}
			\subsubsection{Indentyfikatory}
				\begin{itemize}
					\item Message Id: unikalne id przesyłki nadane przez serwer
					\item In-Reply-to: identyfikator przesyłki do której przesyłający sie odwołuje
					\item References: id jeszcze wcześniejszych przesyłek
					\item Keyword: słowa kluczowe dla danej przesyłki, sprawy
					\item Subject: temat, 1 linijka, podsumowanie przesyłki
					\item X- : dodatkowy tekst na możliwe nowe nagłówki dla konkretnych aplikacji
				\end{itemize}
				...
				\begin{itemize}
					\item 5 dodatkowych nagłówków
					\begin{itemize}
						\item \textbf{MIME-version}: - oznaczenie standardu MIME
						\item \textbf{Content-description}: - słowy opis co to jest za zawartość
						\item \textbf{Content-Id}: - identyfikator zawartości (?)
						\item \textbf{Content-Transfer-Encoding}: - sposób zakodowania przesyłu
						\item \textbf{Content-Type}: - informacja co to za zawartość i jak ją interpretować
					\end{itemize}
					\item Kilka dodatkowych algorytmów związanych z transferem
				\end{itemize}
			\subsubsection{Kodowanie transferu}
				\begin{itemize}
					\item 7-bit ASCII
					\item 8-bit ASCII
					\item BINARY
					\item Dwa ostatnie niosą ze sobą ryzyko związane np. z polskimi znakami.
					\begin{itemize}
						\item BASE-64\\
						3 bajty = 24 bity dzielone na części po 6 bitów.
						\begin{itemize}
							\item a-z
							\item A-Z
							\item 0-9
							...
							\item razem 64 znaki.
						\end{itemize}
						\item Quted Printable Encoding - drukowalne przesłanie.
						\begin{itemize}
							\item Jeżeli w treści listu jest mniej znaków narodowych to przepychamy je inaczej, za pomocą dwóch znaków np.
							\item np. ą = 9Dh = 39 45 = "9", "d"
							\item Jest to algorytm przekodowywania treści
						\end{itemize}
					\end{itemize}
				\end{itemize}

			\subsubsection{Typy zawartości}
				\begin{tabular}{|c|c|}
					\hline Typ & Podtyp \\ 
					\hline Text & Plain, Enriched, HTML, XML \\ 
					\hline Image & gif, jpeg \\ 
					\hline Audio & basic, mpeg (rfe 3003) \\ 
					\hline Video & mpeg \\ 
					\hline Application & octet-stream, postscript, pdf \\ 
					\hline Message & rfe822, partial, external-body \\ 
					\hline Multipart & mixed, alternative, parallel (np. równoczesne odczytywanie video i audio), digest \\ 
					\hline 
				\end{tabular} 

		\subsection{Rozszerzenia MIME}
			\subsubsection{S-MIME}
				Security MIME, stworzone do przekazywania zakodowanych zawartości.\\
				Typy zawartości:
				\begin{tabular}{|c|p{8cm}|}
					\hline Typ & Podtyp \\ 
					\hline multipart & signed (podpisany) \\ 
					\hline application & (dla danych kryptograficznych) pkcs7-mime - signed Data; pkcs7-mime enveloped Data; pkcs7-degenerated signed Data - PKI certyfikat; pkcs7-signature - podpis elektroniczny; pkcs10-mime - żądanie wystawienie certyfikatu (wstawka, która pozwala przekazać dane do systemu, który generuje klucze i tworzy certyfikaty) \\ 
					\hline 
				\end{tabular}
		...
		\subsection{HTML}
			...
			\subsubsection{Metody}
				\begin{itemize}
					\item GET - nazwa pliku + wersja protokołu (np. HTTP/1.0)
					\item HEAD - pozwala na ściągnięcie dokumentu
					\item POST
					\item PUT - załadowanie na serwer dokumentów (zdalna aktualizacja)
					\item DELETE - usuwania wskazanych dokumentów
					\item OPTIONS
					\item TRACE - jak ping, sprawdza czy dokument wysłany do serwera po powrocie nie podlega modyfikacjom
					\item CONNECT - brak zastosowania XDDDDD
					\item PATCH
				\end{itemize}
			\subsubsection{Połączenie}
			Standard zakłada pojedyncze połączenie i rozłączenie się.\\
			\includegraphics[width=9cm]{./images/drawing11.pdf}\\
			Zawiera się tu całość zawarta w <body> ... </body>
			Dokument zawiera szereg nagłówków (\emph{message headers}), które zawierają informacje identyfikujące klienta przed serwerem oraz serwer przed klientem.\\
			udziela informacje jakie wersje MIME jest w stanie obsługiwać, jakiego rozmiaru dane klient może pobierać (jakimi porcjami), mówi też kiedy dokument był modyfikowany.